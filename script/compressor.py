...
Derived from Cats-Team/AdRules script/compressor.py
(https://github.com/Cats-Team/AdRules, 0BSD).
"""
...
except ImportError:  # Support ``python -m script.compressor``.
...
class RuleValidator:
    ...
    def _valid_hostname(self, hostname: str, rule_text: str, has_limit_mod: bool) -> bool:
        ...
        # IP Check
        is_ip = bool(RE_IPV4.fullmatch(hostname))
    ...
    def is_valid(self, rule: str) -> bool:
        if not rule.strip() or self.is_comment(rule):
            return True
        # ``dns_pipeline`` feeds this validator canonical, modifier-free ABP
        # records.  A single anchored output-shape check covers that hot path
        # and avoids the more general parser/hostname checks.  Keep the slow
        # path for arbitrary CLI input and IP validation.
        if rule.startswith("||") and rule.endswith("^"):
            inner = rule[2:-1]
            wildcard_has_public_suffix = not (
                inner.startswith("*.") and "." not in inner[2:]
            )
            if (
                wildcard_has_public_suffix
                and RE_OUTPUT_FILTER.fullmatch(rule) is not None
                and (self.allow_ip or RE_IP_LIKE.fullmatch(inner) is None)
            ):
                return True
    ...
    def validate(self, rules: List[str], keep_removed: bool = True) -> Tuple[List[str], List[str]]:
        """
        Main validation loop (Backwards iteration).
        Returns: (valid_rules, removed_lines_for_logging)
        """
        filtered_reversed: List[str] = []
        removed_log = [] if keep_removed else None
        self._prev_removed = False
        ...
        for line in reversed(rules):
            ...
            if not is_valid_rule:
                ...
                self._prev_removed = False
                filtered_reversed.append(line)

        filtered_reversed.reverse()
        return filtered_reversed, (removed_log if removed_log is not None else [])
...
def compress_rules(rules: List[str], include_wildcards: bool, keep_filtered: bool = True) -> Tuple[List[str], List[str]]:
    """Compress validated rules."""
    seen_hosts: Set[str] = set()
    compressible: List[AdblockRule] = []
    filtered_text: List[str] = []

    def add_filtered(line: str) -> None:
        if keep_filtered:
            filtered_text.append(line)
    kept_wildcards: List[str] = []
    seen_wildcards: Set[str] = set()
    ...
    for line in rules:
        ...
        for r in parsed_list:
            if r.can_compress and r.hostname:
                hostname_key = r.hostname.lower()
                if hostname_key not in seen_hosts:
                    compressible.append(r)
                    seen_hosts.add(hostname_key)
                else:
                    add_filtered(r.original_text)
            else:
                ...
                if include_wildcards and '*' in r.rule_text and not is_cmt:
                    wildcard_key = r.rule_text.lower()
                    if wildcard_key not in seen_wildcards:
    ...
    def has_parent_domain(hostname: str) -> bool:
        hostname = hostname.lower()
        dot_pos = hostname.find('.')
        while dot_pos != -1:
            parent = hostname[dot_pos + 1:]
    ...
    for r in compressible:
        if has_parent_domain(r.hostname):
            add_filtered(r.original_text)
        else:
...
def main() -> int:
    ...
    try:
        if args.input == "-":
            ...
            content = read_utf8_lines(Path(args.input))
    ...
    need_filtered_output = bool(args.filtered)
    valid_content, validation_filtered = validator.validate(content, keep_removed=need_filtered_output)
    
    validated_removed = len(content) - len(valid_content)
    logger.info(f"Validation complete {len(content)} -> {len(valid_content)} (-{validated_removed})")

    # 3. Compression
    compressed, compression_filtered = compress_rules(
        valid_content,
        args.include_wildcards,
        keep_filtered=need_filtered_output,
    )

    all_filtered = validation_filtered + compression_filtered

    compressed_removed = len(content) - len(compressed)
    logger.info(f"Compression complete {len(content)} -> {len(compressed)} (-{compressed_removed})")
    ...
    try:
        if args.in_place and args.input != "-":
            atomic_write_text(Path(args.input), output_lines)
        elif args.output:
            atomic_write_text(Path(args.output), output_lines)
        else:
            sys.stdout.write(output_lines)

        if args.filtered:
            atomic_write_text(Path(args.filtered), "\n".join(all_filtered) + "\n")
